...
const registerHospital = asyncHandler(async (req, res) => {
  ...
  const file = req.file;
  if (!file) {
    throw new ApiError(400, "Hospital logo is required");
  }
  const dataUri = await convertImageToBase64(file);
  const imageCloudinary = await uploadCloudinaryImage(dataUri);
  if (!imageCloudinary) {
...
const logoutHospital = asyncHandler(async (req, res) => {
  ...
  return res
    ...
    .json(new ApiResponse(200, null, "Hospital logged out successfully"));
});
